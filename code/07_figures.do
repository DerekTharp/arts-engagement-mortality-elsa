...
*
* Output: output/figures/figure2_model_comparison.png
*==============================================================================
...
replace order = 3 if model == "Naive confounder-adjusted PH"
...
twoway (rcap ci_low ci_high order, horizontal lcolor(gs4) lwidth(medium)) ///
       ...
       , xline(1, lcolor(gs10) lpattern(dash)) ///
         ...
         note("All estimates are hazard ratios. The four discrete-time panel models" ///
              "share the same monotone-censored sample (cloglog link with interval offset)." ///
              "Error bars = 95% confidence intervals.", size(vsmall)) ///
         legend(off) ///
         graphregion(color(white)) plotregion(margin(l=2 r=12)) ///
         scheme(s2color) ///
         name(forest_labeled, replace)

graph export "$out/figures/figure2_model_comparison.png", name(forest_labeled) replace width(1600)

display "Figure 2 written to $out/figures/figure2_model_comparison.png"
